...
def build_presentation_prompt(
    *,
    ...
    return (
        ...
        "- For clarification mode, put only the exact clarification question in answer_markdown.\n"
        ...
        "- Treat primary_result artifacts as the authoritative result artifact when present.\n"
        "- If Available artifacts does not include primary_visualization, do not mention or reference a visualization artifact.\n"
        "- If a visual recommendation says table and Available artifacts includes primary_result only, reference primary_result rather than a visualization artifact.\n"
        ...
        "- Return artifact_ids as the subset of Available artifact IDs referenced in answer_markdown or explicitly needed for supporting context.\n"
        "- Do not copy result tables, visualization specs, research objects, or summaries into top-level JSON; the runtime owns those as artifacts.\n"
        "- Keep diagnostics as a compact object.\n"
        "- Put response-level metadata in metadata only when it helps the client; do not duplicate the answer.\n"
