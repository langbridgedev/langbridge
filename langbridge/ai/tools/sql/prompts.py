...
SEMANTIC_SQL_ORCHESTRATION_INSTRUCTION = (
    ...
    "- Use DATE_TRUNC for semantic time dimensions when a time bucket is needed.\n"
    "- If the query has a time dimension, add ORDER BY that dimension ASC as the last clause unless a different order is requested.\n"
    ...
    "- Unless the user explicitly requests all rows, apply LIMIT 500 to avoid returning unbounded result sets.\n"
...
DATASET_SQL_ORCHESTRATION_INSTRUCTION = (
    ...
    "- Filter out NULL values on join keys and dimension columns when the query result would otherwise include spurious NULL rows.\n"
    ...
    "- Unless the user explicitly requests all rows, apply LIMIT 500 to avoid returning unbounded result sets.\n"
    "Return SQL in DuckDB dialect only. No comments or explanation."
)
