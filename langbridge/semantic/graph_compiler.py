...
from langbridge.semantic.graph import SemanticGraphRelationship
from langbridge.semantic.model import Metric, Relationship, SemanticModel


@dataclass(frozen=True)
class SemanticGraphSource:
    model: SemanticModel
...
class _SourceModelResolver:
    def __init__(self, source: SemanticGraphSource) -> None:
        self._source = source
    ...
    def resolve(self, member_ref: str) -> _ResolvedMember:
        ...
        if not normalized:
            raise ValueError(f"Semantic graph field reference is empty for '{_source_key(self._source)}'.")
...
def compile_semantic_graph(
    *,
    source_models: Sequence[SemanticGraphSource],
    ...
    for source in source_models:
        ...
        for dataset_key, dataset in source.model.datasets.items():
            ...
            if materialized_key in merged_datasets:
                raise ValueError(
                    f"Duplicate dataset key '{materialized_key}' detected while compiling semantic graph."
                )
    ...
    for relationship_payload in relationships or []:
        relationship = SemanticGraphRelationship.model_validate(dict(relationship_payload))
    ...
    for metric_name, metric_value in (metrics or {}).items():
        ...
        merged_metrics[str(metric_name)] = Metric(
            description=metric.description,
            expression=_rewrite_graph_metric_expression(metric.expression, source_models, resolvers),
        )
    ...
    return semantic_model, dataset_connector_map
...
def _rewrite_source_metric_expression(source: SemanticGraphSource, expression: str) -> str:
...
def _rewrite_graph_metric_expression(
    expression: str,
    source_models: Sequence[SemanticGraphSource],
...
def _source_key(source: SemanticGraphSource) -> str:
...
def _require_source_model_by_id(
    source_models: Sequence[SemanticGraphSource],
    model_id: UUID,
) -> SemanticGraphSource:
    for source_model in source_models:
        if source_model.model_id == model_id:
            return source_model
    raise ValueError(f"Semantic model '{model_id}' is not available in the semantic graph.")
