...
def resolve_dataset_connector_kind(
    *,
    ...
    sync_meta = _connector_sync_meta(file_config)
    sync_connector_kind = _normalize_enum_value(sync_meta.get("connector_type") if sync_meta else None)
    ...
    *,
    ...
    if isinstance(capabilities, DatasetExecutionCapabilities):
        ...
        capability_payload = build_dataset_execution_capabilities(
            source_kind=DatasetSourceKind(str(source_kind or DatasetSourceKind.FILE.value).lower()),
            storage_kind=DatasetStorageKind(str(storage_kind or DatasetStorageKind.CSV.value).lower()),
...
def derive_legacy_dataset_type(
    *,
    ...
    if source_value == DatasetSourceKind.VIRTUAL.value or storage_value == DatasetStorageKind.VIRTUAL.value:
        return "FEDERATED"
    if storage_value in {DatasetStorageKind.CSV.value, DatasetStorageKind.PARQUET.value, DatasetStorageKind.JSON.value}:
        return "FILE"
...
def relation_reference_candidates(relation_identity: DatasetRelationIdentity | Mapping[str, Any] | None) -> set[str]:
    if relation_identity is None:
        return set()
    if not isinstance(relation_identity, DatasetRelationIdentity):
        try:
            relation_identity = DatasetRelationIdentity.model_validate(dict(relation_identity))
        except Exception:
            return set()

    candidates = {
        str(relation_identity.relation_name or "").strip().lower(),
        str(relation_identity.qualified_name or "").strip().lower(),
        str(relation_identity.table_name or "").strip().lower(),
    }
    if relation_identity.schema_name and relation_identity.table_name:
        candidates.add(f"{relation_identity.schema_name}.{relation_identity.table_name}".lower())
