...
def test_runtime_host_api_exposes_runtime_features(tmp_path: Path) -> None:
    ...
    agent = client.post(
        ...
        json={
            ...
            "metadata_json": {"follow_up": True, "surface": "direct"},
        },
    )
    ...
    assert messages.json()["items"][0]["content"]["metadata_json"] == {"follow_up": True, "surface": "direct"}
    assert messages.json()["items"][0]["content"]["context"]["metadata_json"] == {
        "follow_up": True,
        "surface": "direct",
    }
    assert messages.json()["items"][0]["model_snapshot"]["metadata_json"] == {
        "follow_up": True,
        "surface": "direct",
    }
...
def test_runtime_host_api_streams_agent_run_progress_and_persists_final_message(tmp_path: Path) -> None:
    ...
    assert messages.json()["items"][0]["content"]["metadata_json"] == {"streaming": True}
                ...
                client.post(
                    ...
                    json={
                        ...
                        "query": (
                            ...
                            "FROM shopify_orders AS o "
                            "JOIN shopify_customers AS c ON o.customer_id = c.customer_id "
                            "WHERE REGEXP_MATCHES(c.region, 'Europe|North America') "
                            "GROUP BY c.region "
                            "ORDER BY net_sales DESC"
                        ),
                    },
