...
def test_persist_supervisor_state_clears_resolved_clarification_state() -> None:
    ...
    assert thread.metadata_json == {}


class _CloudThreadState(enum.Enum):
    awaiting_user_input = "awaiting_user_input"
    processing = "processing"


def test_set_thread_awaiting_user_input_preserves_loaded_enum_type() -> None:
    thread = SimpleNamespace(state=_CloudThreadState.processing)

    AgentExecutionService._set_thread_awaiting_user_input(thread)

    assert thread.state is _CloudThreadState.awaiting_user_input
